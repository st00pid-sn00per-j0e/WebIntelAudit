...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/scans', async (req, res) => {
    try {
      ...
      // Start the Python appium analyzer process (Chrome WebDriver)
      const pythonScript = path.join(process.cwd(), 'server', 'services', 'appium_analyzer.py');
      console.log(`Starting Chrome-based analysis for session ${session.id}: ${pythonScript}`);
